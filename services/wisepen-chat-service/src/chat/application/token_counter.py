...
class TokenCounter:
    ...
    async def count_text(self, text: str, model_name: str = "gpt-4o") -> int:
        try:
            return await asyncio.to_thread(litellm.token_counter, model=model_name, text=text)
        except Exception:
    ...
    async def count_messages(
        ...
        try:
            return await asyncio.to_thread(litellm.token_counter, model=model_name, messages=payload, tools=tools)
        except Exception:
            if tools:
