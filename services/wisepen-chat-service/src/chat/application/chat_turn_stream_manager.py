...
class ChatTurnStreamManager:
    ...
    async def start_turn(
        ...
        *,
        user_id: str,
        session_id: str,
        build_chat_gen: Callable[[BackgroundTasks], AsyncIterator[str]],
    ) -> str:
        ...
        self._tasks.add(task) # 避免后台任务变成无人持有的悬空任务
        task.add_done_callback(self._tasks.discard) # 任务结束后自动移除，避免内存泄漏
        return turn_id
    ...
    async def _run_turn(
        ...
        *,
        ...
        try:
            ...
            async for frame in build_chat_gen(background_tasks):
