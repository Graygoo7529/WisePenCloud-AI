...
提供商的 API 密钥（API keys）通过 MCP 请求元数据（Metadata）传入。
...
`search_academic()` 可以回退（Fall back）到普通 Web 搜索。仅当提供商拥有真实的学术端点（Academic endpoint）或已文档化的研究筛选器（Research filter）时，才可重写该方法。